...
export function ActionTrendChart({
  ...
  data: { label: string; tooltipLabel: string; booked: number; confirmed: number; cancelled: number }[];
  granularityLabel: string;
}) {
  const hasData = data.some((row) => row.booked > 0 || row.confirmed > 0 || row.cancelled > 0);

  return (
    <Card>
      <CardHeader>
        <CardTitle>Action Totals</CardTitle>
        <CardDescription>{granularityLabel[0].toUpperCase() + granularityLabel.slice(1)} booked, confirmed, and cancelled totals</CardDescription>
      </CardHeader>
      <CardContent>
        {!hasData ? (
          ...
          <ChartContainer config={chartConfig} className="h-[220px] w-full">
            <BarChart data={data}>
              ...
              <Bar dataKey="cancelled" stackId="actions" fill="var(--color-cancelled)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ChartContainer>
        )}
      </CardContent>
