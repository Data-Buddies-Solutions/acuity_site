...
const transferCall: CallView = {
  ...call,
  answeredAt: "2026-07-14T12:00:05.000Z",
  legs: [
    {
...
describe("selectIncomingRingtoneOffer", () => {
  it("selects only this browser session's exact offered inbound call", () => {
    expect(select()).toBe(call.id);
    expect(
      select({ selectedSession: { ...session, offeredCallId: "call-2" } }),
    ).toBeNull();
  ...
  it("stops on answer promotion, reconnect, or Ready and media loss", () => {
    expect(
  ...
  it("rings for a transfer only while its exact source, session, endpoint, and leg are live", () => {
    expect(select({ calls: [transferCall], operations: [transferOperation] })).toBe(
      call.id,
    );
    expect(
      select({
    ...
    expect(
      select({
        calls: [transferCall],
        operations: [{ ...transferOperation, targetEndpointId: "endpoint-2" }],
      }),
    ).toBeNull();
    expect(
      select({
        calls: [transferCall],
        operations: [{ ...transferOperation, sourceLegId: "other-winner" }],
      }),
    ).toBeNull();
    expect(
      select({
        calls: [
          {
            ...transferCall,
            legs: transferCall.legs.map((leg) =>
              leg.id === "target-leg" ? { ...leg, status: "ENDED" } : leg,
            ),
          },
        ],
  ...
  it("stops when another transfer winner replaces the source", () => {
    expect(
      select({
        calls: [
          {
            ...transferCall,
            legs: transferCall.legs.map((leg) =>
              leg.id === "target-leg" ? { ...leg, status: "BRIDGED" } : leg,
            ),
  ...
  it("does not restore a withdrawn offer from a stale realtime event", () => {
...
describe("useIncomingCallRingtone", () => {
  ...
  it("uses one looping audio instance without restarting on repeated updates", async () => {
  ...
  it("stops on clear and rings for a later offer", async () => {
    const { rerender } = renderHook(({ offerId }) => useIncomingCallRingtone(offerId), {
  ...
  it("shows recovery after autoplay rejection and retries from the operator action", async () => {
