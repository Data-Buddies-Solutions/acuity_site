  ...
  return Boolean(
    operations?.some((operation) => {
      if (
        ...
        operation.sourceLegId !== call.winningLegId ||
      ...
      const targetLeg = call.legs.find(({ id }) => id === operation.targetLegId);
      return (
        targetLeg?.kind === "AGENT" &&
        targetLeg.agentSessionId === session.id &&
        targetLeg.endpointId === session.endpointId &&
        !["ENDED", "FAILED"].includes(targetLeg.status)
      );
    }),
  );
}
...
export function selectIncomingRingtoneOffer({
  ...
  if (
    ...
    (!ACTIONABLE_CALL_STATUSES.has(call.status) &&
      !isExactTransferOffer(call, operations, session))
  ) {
    return null;
  }
