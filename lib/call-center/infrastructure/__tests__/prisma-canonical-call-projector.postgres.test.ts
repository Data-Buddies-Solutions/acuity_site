...
describePostgres("canonical call projector on PostgreSQL", () => {
  ...
  it("processes concurrent bridges for different Calls sharing one endpoint without deadlock", async () => {
    const fixture = await createFixture(prisma);
    const firstCallId = fixture.id("shared-endpoint-first-call");
    const secondCallId = fixture.id("shared-endpoint-second-call");
    const firstLegId = fixture.id("shared-endpoint-first-leg");
    const secondLegId = fixture.id("shared-endpoint-second-leg");
    const firstBridge = await fixture.processingEvent(
      "call.bridged",
      "shared-endpoint-first",
    );
    const secondBridge = await fixture.processingEvent(
      "call.bridged",
      "shared-endpoint-second",
    );

    try {
      await prisma.callCenterCall.createMany({
        data: [firstCallId, secondCallId].map((id) => ({
          deadlineAt: new Date(occurredAt.getTime() + 20_000),
      ...
      await prisma.callCenterCallLeg.createMany({
        data: [
          {
            callId: firstCallId,
            id: fixture.id("shared-endpoint-first-customer"),
            kind: "CUSTOMER",
            providerCallControlId: fixture.id("shared-endpoint-first-customer-control"),
            providerCallLegId: fixture.id("shared-endpoint-first-customer-provider-leg"),
            startedAt: occurredAt,
            status: "ANSWERED",
          },
          {
            callId: secondCallId,
            id: fixture.id("shared-endpoint-second-customer"),
            kind: "CUSTOMER",
            providerCallControlId: fixture.id("shared-endpoint-second-customer-control"),
            providerCallLegId: fixture.id("shared-endpoint-second-customer-provider-leg"),
            startedAt: occurredAt,
            status: "ANSWERED",
          },
          {
            ...
            id: firstLegId,
            kind: "AGENT",
            providerCallControlId: fixture.id("shared-endpoint-first-control"),
            providerCallLegId: fixture.id("shared-endpoint-first-provider-leg"),
            startedAt: occurredAt,
            status: "RINGING",
          },
          {
            ...
            id: secondLegId,
            kind: "AGENT",
            providerCallControlId: fixture.id("shared-endpoint-second-control"),
            providerCallLegId: fixture.id("shared-endpoint-second-provider-leg"),
            startedAt: occurredAt,
            status: "RINGING",
          },
        ],
      ...
      await Promise.all([
        projector.projectAndComplete(
          ...
          fixture.fact({
            ...
            providerCallControlId: fixture.id("shared-endpoint-first-control"),
            providerCallLegId: fixture.id("shared-endpoint-first-provider-leg"),
            providerCallSessionId: fixture.id("shared-endpoint-first-session"),
            providerEventId: firstBridge.providerEventId,
          }),
          projectedAt,
        ),
        projector.projectAndComplete(
          ...
          fixture.fact({
            ...
            providerCallControlId: fixture.id("shared-endpoint-second-control"),
            providerCallLegId: fixture.id("shared-endpoint-second-provider-leg"),
            providerCallSessionId: fixture.id("shared-endpoint-second-session"),
            providerEventId: secondBridge.providerEventId,
          }),
          projectedAt,
        ),
      ...
      const calls = await prisma.callCenterCall.findMany({
        ...
        orderBy: { id: "asc" },
        where: { id: { in: [firstCallId, secondCallId] } },
      });
      expect(calls.filter(({ status }) => status === "CONNECTED")).toHaveLength(1);
      expect(calls.filter(({ winningLegId }) => winningLegId !== null)).toHaveLength(1);
      expect(
        calls.flatMap(({ legs }) => legs).filter(({ status }) => status === "BRIDGED"),
      ).toHaveLength(1);
      expect(
        calls.flatMap(({ legs }) => legs).filter(({ status }) => status === "ENDED"),
      ).toHaveLength(1);
      expect(
        await prisma.providerWebhookEvent.count({
          where: {
  ...
  it("processes a bridge while another Call claims the same endpoint without deadlock", async () => {
    const fixture = await createFixture(prisma);
    const agent = await fixture.createReadyAgent();
    const bridgedCallId = fixture.id("claim-race-bridged-call");
    const claimedCallId = fixture.id("claim-race-claimed-call");
    const bridgedLegId = fixture.id("claim-race-bridged-leg");
    const claimedLegId = fixture.id("claim-race-claimed-leg");
    const bridge = await fixture.processingEvent("call.bridged", "claim-race");

    try {
      await prisma.callCenterCall.createMany({
        data: [bridgedCallId, claimedCallId].map((id) => ({
          deadlineAt: new Date(projectedAt.getTime() + 20_000),
          direction: "INBOUND" as const,
          fromPhone: fixture.callerPhone,
          hardDeadlineAt: new Date(projectedAt.getTime() + 60_000),
          id,
          numberId: fixture.numberId,
          practiceId: fixture.practiceId,
          queueId: fixture.queueId,
        ...
        data: [
          {
            ...
            id: fixture.id("claim-race-bridged-customer"),
            kind: "CUSTOMER",
            providerCallControlId: fixture.id("claim-race-bridged-customer-control"),
            startedAt: occurredAt,
            status: "ANSWERED",
          },
          {
            callId: claimedCallId,
            id: fixture.id("claim-race-claimed-customer"),
            kind: "CUSTOMER",
            providerCallControlId: fixture.id("claim-race-claimed-customer-control"),
            startedAt: occurredAt,
            status: "ANSWERED",
          },
          {
            ...
            id: bridgedLegId,
            kind: "AGENT",
            providerCallControlId: fixture.id("claim-race-bridged-control"),
            providerCallLegId: fixture.id("claim-race-bridged-provider-leg"),
            startedAt: occurredAt,
            status: "RINGING",
          },
          {
            ...
            id: claimedLegId,
            kind: "AGENT",
            providerCallControlId: fixture.id("claim-race-claimed-control"),
            providerCallLegId: fixture.id("claim-race-claimed-provider-leg"),
            startedAt: occurredAt,
            status: "RINGING",
          },
        ],
      ...
      const claimStore = new PrismaInboundAnswerClaimStore((work) =>
        prisma.$transaction(work),
      );
      const [claim, projection] = await Promise.all([
        claimInboundAnswer(
          claimStore,
          {
            allowedLocationIds: [fixture.locationId],
          ...
          {
            callId: claimedCallId,
            idempotencyKey: fixture.id("claim-race-answer"),
            legId: claimedLegId,
        ...
        projector.projectAndComplete(
          bridge,
          fixture.fact({
            canonicalCallId: bridgedCallId,
            canonicalLegId: bridgedLegId,
            endpointId: fixture.endpointId,
            eventType: "call.bridged",
            legKind: "AGENT",
            providerCallControlId: fixture.id("claim-race-bridged-control"),
            providerCallLegId: fixture.id("claim-race-bridged-provider-leg"),
            providerEventId: bridge.providerEventId,
          }),
          projectedAt,
        ),
      ]);

      expect(["ACCEPTED", "REJECTED"]).toContain(claim.status);
