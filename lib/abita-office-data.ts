import hollywoodSweetwaterInsuranceRules from "../data/abita-hollywood-sweetwater-insurance.json";

export const ABITA_HOLLYWOOD_MARKDOWN = `# Knowledge Base: Abita Eye Group Hollywood

## Emergency Notice

If this is a medical emergency, please hang up and dial 911 or go to the nearest emergency room immediately.

## Urgency Screening

To determine the right scheduling lane, ask what the visit is for.

Route to ophthalmology if the caller reports any current vision issue or medical eye concern, including:
- Sudden loss of vision or major vision change
- Flashes
- New floaters
- Redness or infected eye
- Eye injury
- Eye pain
- Cataract, glaucoma, retina injections, diabetic eye care, or thyroid eye disease
- Surgery consult or post-op issue
- Referral from another provider
- Ongoing medical eye condition

Route to optometry if the caller wants:
- Routine eye exam for glasses
- Contact lens exam
- Updating a glasses or contact lens prescription
- Help with glasses or contact lenses that were already ordered

Hollywood does not perform retina surgical care. Retina surgical care needs to be redirected to a retina surgeon. If the caller does not have a retina surgeon, tell them to contact their primary care provider or insurance company so they can be referred to the appropriate physician or practice.

## Location + Contact

Practice Name: Abita Eye Group Hollywood
Address: 4330 Sheridan St, Suite 102B, Hollywood, FL
Email: newpatient@abitaeye.com
Phone: (954) 287-2010

Hours: Monday-Thursday 8:30 AM-4:30 PM. Closed for lunch 12:30 PM-1:30 PM. Friday 8:30 AM-2:30 PM.

## Scope of Services

Hollywood sees medical eye conditions and ophthalmology visits through Dr. Bach's medical lane. Hollywood also has an optometry lane for routine eye exams, eyeglass prescriptions, contact lens fittings, and insertion training.

Routine-vision age minimums follow the scheduler provider rules: Dr. Kyler Farnan sees routine-vision patients age 5 and older, Dr. Lisbet Vidal sees routine-vision patients age 7 and older, and Dr. Gisselle Calero sees routine-vision patients age 4 and older. If no routine-vision provider is age-eligible, route the patient to Dr. Bach, who is a pediatric ophthalmologist.

## Providers - Hollywood

Doctors at the Hollywood location:
- Dr. Calero
- Dr. Vidal
- Dr. Bach
- Dr. Farnan

Medical scheduling:
- Dr. Austin Bach
- Dr. Bach overflow

Routine vision scheduling:
- Dr. Kyler Farnan sees routine-vision patients age 5 and older.
- Dr. Lisbet Vidal sees routine-vision patients age 7 and older.
- Dr. Gisselle Calero sees routine-vision patients age 4 and older.

## Optical / Glasses

Hollywood stocks many major frame brands, including Ray-Ban, Nike, Gucci, Chloe, Montblanc, Puma, Cartier, Lindberg, Mykita, and YSL.

Lens upgrades and treatments are available. The office offers all types of lenses, including upgrades and treatments.

Eyeglass prescriptions are valid for up to 2 years, but a yearly eye exam is recommended to make sure the prescription is current.

If a patient wants glasses or contacts and already has a prescription, they can bring the prescription from their optometrist and choose from the office's frame selection. If they need a prescription, schedule them with the optometrist first, then they can browse frames and lenses.

Glasses are usually ready in less than 10 business days.

Non-prescription sunglasses are available for purchase. If the patient wears prescription glasses, the office can customize prescription lenses into those frames. Most vision insurance plans do not cover non-prescription sunglasses. Sunglass brands include Maui Jim, Costa, Ray-Ban, Gucci, and many more.

## Contact Lenses

Ask whether the patient has worn contact lenses in the past.

If this is the patient's first set of contact lenses, the exam and insertion training take longer than a standard visit. First-time contact lens appointments must be scheduled before 3:00 PM Monday-Thursday and before 1:00 PM on Friday.

## Licensed Optician

Katie is the licensed optician. She has over 5 years of eyecare experience and specializes in frames, lenses, and fittings. She can also complete minor repairs in the office at no cost when possible.

No appointment is needed to look at glasses or have current glasses adjusted. Sasha and Denise are in the office Monday-Thursday 8:30 AM-4:30 PM and Friday 8:30 AM-2:30 PM.

## Insurance

Hollywood has its own medical insurance list in the agent insurance rules. Use \`check_insurance\` with coverageType \`medical\` for medical or surgical visits.

Hollywood also supports routine vision. For routine eye exams, glasses prescriptions, or contact lens prescriptions using vision insurance, use \`check_insurance\` with coverageType \`routine_vision\`, then schedule with routing \`optical_only\`.

## What to Bring

Photo ID, insurance card, list of current medications, and previous eye records if available.

## Appointment Expectations

New patient visits may take 1-2 hours depending on testing. Eye dilation may occur and can cause temporary blurry vision and light sensitivity.

## Social Follow-Up

At the end of Hollywood calls, ask the caller to follow Abita Eye Group on Facebook and Instagram at @abitaeyegroup.

## Scope Limitation

This knowledge base provides general practice information only. It does not provide diagnosis, medical advice, or treatment recommendations.`;

export const ABITA_SWEETWATER_MARKDOWN = `# Knowledge Base: Abita Eye Group Sweetwater

## Emergency Notice

If this is a medical emergency, please hang up and dial 911 or go to the nearest emergency room immediately.

## Urgency Screening

To determine the right scheduling lane, ask what the visit is for.

Route to ophthalmology if the caller reports any current vision issue or medical eye concern, including:
- Sudden loss of vision or major vision change
- Flashes
- New floaters
- Redness or infected eye
- Eye injury
- Eye pain
- Cataract, glaucoma, retina injections, diabetic eye care, or thyroid eye disease
- Surgery consult or post-op issue
- Referral from another provider
- Ongoing medical eye condition

Route to optometry if the caller wants:
- Routine eye exam for glasses
- Contact lens exam
- Updating a glasses or contact lens prescription
- Help with glasses or contact lenses that were already ordered

Sweetwater does not perform retina surgical care. Retina surgical care needs to be redirected to a retina surgeon. If the caller does not have a retina surgeon, tell them to contact their primary care provider or insurance company so they can be referred to the appropriate physician or practice.

## Location + Contact

Practice Name: Abita Eye Group Sweetwater
Address: 12750 NW 17th St, #201, Miami, FL 33182
Email: newpatient@abitaeye.com
Phone: (786) 465-4836

Hours: Monday-Thursday 8:30 AM-4:30 PM. Friday 8:30 AM-2:30 PM.

## Scope of Services

Sweetwater sees medical eye conditions and ophthalmology visits through the medical lane. Sweetwater also has an optometry lane for routine eye exams, eyeglass prescriptions, contact lens fittings, and insertion training.

Routine-vision age minimums follow the scheduler provider rules: Dr. Maria Casas sees routine-vision patients age 7 and older, Dr. Kyler Farnan sees routine-vision patients age 5 and older, and Dr. Gisselle Calero sees routine-vision patients age 4 and older. If no routine-vision provider is age-eligible, route the patient to Dr. Bach, who is a pediatric ophthalmologist.

## Providers - Sweetwater

Doctors at the Sweetwater location:
- Dr. Gisselle Calero
- Dr. Kyler Farnan
- Dr. Maria Casas

Medical scheduling:
- Dr. Austin Bach
- Dr. Bach overflow

Routine vision scheduling:
- Dr. Maria Casas sees routine-vision patients age 7 and older.
- Dr. Kyler Farnan sees routine-vision patients age 5 and older.
- Dr. Gisselle Calero sees routine-vision patients age 4 and older.

## Optical / Glasses

Sweetwater stocks many major frame brands, including Ray-Ban, Nike, Gucci, Chloe, Montblanc, Puma, Cartier, Lindberg, Mykita, and YSL.

Lens upgrades and treatments are available. The office offers all types of lenses, including upgrades and treatments.

Eyeglass prescriptions are valid for up to 2 years, but a yearly eye exam is recommended to make sure the prescription is current.

If a patient wants glasses or contacts and already has a prescription, they can bring the prescription from their optometrist and choose from the office's frame selection. If they need a prescription, schedule them with the optometrist first, then they can browse frames and lenses.

Glasses are usually ready in less than 10 business days.

Non-prescription sunglasses are available for purchase. If the patient wears prescription glasses, the office can customize prescription lenses into those frames. Most vision insurance plans do not cover non-prescription sunglasses. Sunglass brands include Maui Jim, Costa, Ray-Ban, Gucci, and many more.

## Contact Lenses

Ask whether the patient has worn contact lenses in the past.

If this is the patient's first set of contact lenses, the exam and insertion training take longer than a standard visit. First-time contact lens appointments must be scheduled before 3:00 PM Monday-Thursday and before 1:00 PM on Friday.

## Licensed Optician

Betty is the licensed optician. She has over 30 years of eyecare experience and specializes in frames, lenses, and fittings. She can also complete minor repairs in the office at no cost when possible.

No appointment is needed to look at glasses or have current glasses adjusted. Betty and Oliva are in the office Monday-Friday 8:30 AM-4:30 PM.

## Insurance

Sweetwater has its own medical insurance list in the agent insurance rules. Use \`check_insurance\` with coverageType \`medical\` for medical or surgical visits.

Sweetwater also supports routine vision. For routine eye exams, glasses prescriptions, or contact lens prescriptions using vision insurance, use \`check_insurance\` with coverageType \`routine_vision\`, then schedule with routing \`optical_only\`.

## What to Bring

Photo ID, insurance card, list of current medications, and previous eye records if available.

## Appointment Expectations

New patient visits may take 1-2 hours depending on testing. Eye dilation may occur and can cause temporary blurry vision and light sensitivity.

## Social Follow-Up

At the end of Sweetwater calls, ask the caller to follow Abita Eye Group on Facebook and Instagram at @abitaeyegroup.

## Scope Limitation

This knowledge base provides general practice information only. It does not provide diagnosis, medical advice, or treatment recommendations.`;

export const ABITA_HOLLYWOOD_SWEETWATER_INSURANCE_RULES =
  hollywoodSweetwaterInsuranceRules;

export type AbitaNewOfficeKey = "hollywood" | "sweetwater";

export type AbitaNewOfficeConfig = {
  key: AbitaNewOfficeKey;
  address: string;
  aliases: string[];
  email: string;
  hoursSummary: string;
  insuranceTitle: string;
  knowledgeMarkdown: string;
  knowledgeTitle: string;
  name: string;
  phones: Array<{
    isPrimary: boolean;
    label: string;
    phoneNumber: string;
  }>;
  primaryPhone: string;
  ruleSlug: string;
  documentSlug: string;
};

export const ABITA_NEW_OFFICES = [
  {
    address: "4330 Sheridan St, Suite 102B, Hollywood, FL",
    aliases: ["hollywood"],
    documentSlug: "abita-eye-group-hollywood",
    email: "newpatient@abitaeye.com",
    hoursSummary:
      "Monday-Thursday 8:30 AM-4:30 PM. Closed for lunch 12:30 PM-1:30 PM. Friday 8:30 AM-2:30 PM.",
    insuranceTitle: "Insurance Rules: Abita Eye Group, Hollywood",
    key: "hollywood",
    knowledgeMarkdown: ABITA_HOLLYWOOD_MARKDOWN,
    knowledgeTitle: "Knowledge Base: Abita Eye Group Hollywood",
    name: "Hollywood",
    phones: [
      {
        isPrimary: true,
        label: "Hollywood",
        phoneNumber: "+19542872010",
      },
    ],
    primaryPhone: "+19542872010",
    ruleSlug: "hollywood-insurance-rules",
  },
  {
    address: "12750 NW 17th St, #201, Miami, FL 33182",
    aliases: ["sweetwater"],
    documentSlug: "abita-eye-group-sweetwater",
    email: "newpatient@abitaeye.com",
    hoursSummary: "Monday-Thursday 8:30 AM-4:30 PM. Friday 8:30 AM-2:30 PM.",
    insuranceTitle: "Insurance Rules: Abita Eye Group, Sweetwater",
    key: "sweetwater",
    knowledgeMarkdown: ABITA_SWEETWATER_MARKDOWN,
    knowledgeTitle: "Knowledge Base: Abita Eye Group Sweetwater",
    name: "Sweetwater",
    phones: [
      {
        isPrimary: false,
        label: "Sweetwater 786-465-7475",
        phoneNumber: "+17864657475",
      },
      {
        isPrimary: false,
        label: "Sweetwater 786-465-4845",
        phoneNumber: "+17864654845",
      },
      {
        isPrimary: false,
        label: "Sweetwater 786-613-4310",
        phoneNumber: "+17866134310",
      },
      {
        isPrimary: false,
        label: "Sweetwater 786-465-7479",
        phoneNumber: "+17864657479",
      },
      {
        isPrimary: true,
        label: "Sweetwater",
        phoneNumber: "+17864654836",
      },
      {
        isPrimary: false,
        label: "Sweetwater 786-465-4882",
        phoneNumber: "+17864654882",
      },
    ],
    primaryPhone: "+17864654836",
    ruleSlug: "sweetwater-insurance-rules",
  },
] as const satisfies readonly AbitaNewOfficeConfig[];

export function findAbitaNewOfficeByLocation(location: {
  address: string | null;
  name: string;
}) {
  const haystack = `${location.name} ${location.address || ""}`.toLowerCase();
  return (
    ABITA_NEW_OFFICES.find((office) =>
      office.aliases.some((alias) => haystack.includes(alias)),
    ) ?? null
  );
}
