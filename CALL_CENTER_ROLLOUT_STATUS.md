...
Draft PR [#108](https://github.com/Data-Buddies-Solutions/acuity_site/pull/108)
wires the portal to exactly one server-authorized `SHADOW` queue. It claims a
...
| 5A    | Canonical snapshot, ordered SSE, reducer, and media adapter              | #107 ready; #108 shadow shell in draft            | Legacy UI remains authoritative       |
...
| Coordinated call control | Idempotent commands, ordered SSE, reducer, and media adapter | #103-#107 ready; #108 shadow shell draft | Build active operations/media/actions, then activate 4B/5B together |
...
- PRs #103-#107 have green Prisma, format, lint, typecheck, test, build, and
  Vercel checks. PR #108 locally passes the full test suite, focused
  changed-file lint, full typecheck, Prisma validation, and the optimized
...
3. land canonical claim, transfer, outbound, disposition, voicemail/failover,
   session-bound media credentials, and the action UI;
